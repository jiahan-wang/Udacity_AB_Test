...
End-to-end A/B test evaluation of Udacity's free-trial screener (the weekly-time-investment gate shown before checkout), on the course's daily-aggregated dataset: a 37-day traffic window and a 23-day outcome window (14-day payment lag), Control 345,543 pageviews / 28,378 clicks vs Experiment 344,660 / 28,325.

**Pipeline**: cleaning & dual-window handling → experiment quality (SRM χ² p ≈ 0.29 with exact binomial; invariant metrics incl. CTP; daily trend diagnostics) → design & precision (locked MDE 1.0 pp Gross / 0.75 pp Net; power at the achieved sample only 0.64 / 0.62 → underpowered) → effect estimation (two-proportion Z with pooled test / unpooled CI; day-cluster bootstrap; delta method) → aggregate-data permutation robustness (same-weekday blocks; explicitly not an AA test) → non-inferiority decision on Net Conversion against a pre-locked δ = 0.75 pp → cost-benefit scenario analysis.

**Engineering**: every result artifact is regenerated by one pipeline from SHA-256-verified raw data with fixed seeds; the test suite includes a src-vs-results regression (deterministic values matched to 1e-12) and an end-to-end rerun in a temp directory; CI runs everything on every push after fetching the data; an independent recompute script sharing no code with the pipeline re-derives 25 key numbers; a Streamlit dashboard exposes δ and cost assumptions as sensitivity-only sliders.

**Why it stands out**: the screener works as designed on enrollments — Gross Conversion drops 2.06 pp (p = 2.6e-6) — but the launch decision hinges on Net Conversion, whose −0.49 pp effect is not significant and whose 95% CI (−1.16, +0.19 pp) crosses the non-inferiority boundary, so non-inferiority is *not established* (inconclusive, not proof of harm). Rather than over-claiming in either direction, the repo reports the power shortfall head-on, treats the day-cluster caveat (cluster SE 2.6–3.0× the iid SE; the Gross CI crosses 0) as a real limitation, keeps cost savings as a clearly labelled scenario range (Base ≈ 2,661 USD per 23 days, cluster interval −617 to 5,938), and lands on "continue experiment" instead of a premature launch-or-kill call.

**Reproduce**: `python scripts/download_data.py` → `python scripts/run_pipeline.py` → `python -m pytest -q`; dashboard: `streamlit run app/streamlit_app.py`. Raw CSVs are not committed (license); the download script fetches them and verifies SHA-256.
